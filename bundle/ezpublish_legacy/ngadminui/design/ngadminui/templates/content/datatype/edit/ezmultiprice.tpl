...
<div class="block">
    ...
    {def $multi_price = $attribute.content
            ...
            <table class="list" cellspacing="0">
                ...
                {foreach $custom_price_list as $price}
                    <tr>
                        ...
                        {* Value *}
                        <td><input type="text" class="form-control input-sm" name="{$attribute_base}_price_array_{$attribute.id}[{$price.currency_code}]" size="12" value="{$price.value}" /></td>
                    </tr>
                {/foreach}
            </table>

            {* 'Remove' button *}
            <input class="btn btn-default" type="submit" name="CustomActionButton[{$attribute.id}_remove_prices]" value="{'Remove selected'|i18n( 'design/standard/content/datatype' )}" title="{'Remove selected prices.'|i18n( 'design/standard/content/datatype' )}" /><p>
        {else}
        ...
        {if count( $currency_list )}
            {* Select currency *}
            <select name="{$attribute_base}_selected_currency_{$attribute.id}" class="form-control inline" title="Select currency">
            ...
            {* 'Set price' button *}
            <input class="btn btn-default" type="submit" name="CustomActionButton[{$attribute.id}_set_custom_price]" value="{'Set custom price'|i18n( 'design/standard/content/datatype' )}" title="{'Set custom price.'|i18n( 'design/standard/content/datatype' )}" />
...
<div class="block form-inline">
    ...
    <select id="ezcoa-{if ne( $attribute_base, 'ContentObjectAttribute' )}{$attribute_base}-{/if}{$attribute.contentclassattribute_id}_{$attribute.contentclass_attribute_identifier}_inc_ex_vat" class="form-control ezcc-{$attribute.object.content_class.identifier} ezcca-{$attribute.object.content_class.identifier}_{$attribute.contentclass_attribute_identifier}" name="{$attribute_base}_ezmultiprice_inc_ex_vat_{$attribute.id}">
