...
{if $class_content.selection_type|ne( 0 )} {* If current selection mode is not 'browse'. *}
        ...
        {case match=1} {* Dropdown list *}
            <div class="buttonblock clearfix">
                ...
                <select class="form-control inline" name="{$attribute_base}_data_object_relation_list_{$attribute.id}[]">
        ...
        {case match=2} {* radio buttons list *}
            ...
            {if $attribute.contentclass_attribute.is_required|not}
                <div class="radio">
                    <label><input type="radio" name="{$attribute_base}_data_object_relation_list_{$attribute.id}[]" value="no_relation" {if eq( $attribute.content.relation_list|count, 0 )} checked="checked"{/if}>{'No relation'|i18n( 'design/standard/content/datatype' )}</label>
                </div>
            {/if}
            {section var=node loop=$nodesList}
                <div class="radio">
                    <label><input type="radio" name="{$attribute_base}_data_object_relation_list_{$attribute.id}[]" value="{$node.contentobject_id}"
                    {if ne( count( $attribute.content.relation_list ), 0)}
                    {foreach $attribute.content.relation_list as $item}
                         {if eq( $item.contentobject_id, $node.contentobject_id )}
                                checked="checked"
        ...
        {case match=3} {* check boxes list *}
            {section var=node loop=$nodesList}
                <div class="checkbox">
                    <label><input type="checkbox" name="{$attribute_base}_data_object_relation_list_{$attribute.id}[{$node.node_id}]" value="{$node.contentobject_id}"
                    {if ne( count( $attribute.content.relation_list ), 0)}
                    {foreach $attribute.content.relation_list as $item}
                         {if eq( $item.contentobject_id, $node.contentobject_id )}
                                checked="checked"
        ...
        {case match=4} {* Multiple List *}
            <div class="buttonblock">
                {if ne( count( $nodesList ), 0)}
                <select class="form-control form-control-multiple inline" name="{$attribute_base}_data_object_relation_list_{$attribute.id}[]" size="10" multiple>
                    {section var=node loop=$nodesList}
                        <option value="{$node.contentobject_id}"
                        {if ne( count( $attribute.content.relation_list ), 0)}
                        {foreach $attribute.content.relation_list as $item}
...
{else}    {* Standard mode is browsing *}
    ...
    {else}
        ...
        <table class="list{if $attribute.content.relation_list|not} hide{/if}" cellspacing="0">
            ...
            {else}
              <tr class="bgdark">
                ...
                <td>--section-name--</td>
                <td>--published--</td>
                <td><input size="2" type="text" class="form-control" name="{$attribute_base}_priority[{$attribute.id}][]" value="0" /></td>
              </tr>
            {/if}
            </tbody>
        </table>
        ...
        <div class="right input-group">
            <input type="text" class="form-control ezobject-relation-search-text" />
            <span class="input-group-btn">
                <button type="submit" class="btn btn-default ezobject-relation-search-btn" name="CustomActionButton[{$attribute.id}_browse_objects]"><i class="fa fa-search"></i> {'Find objects'|i18n( 'design/standard/content/datatype' )}</button>
            </span>
        </div>
        <div class="break"></div>
