...
{/section}
        {section show=$MultiOptionList.item.imageoption|not()}
            <select class="form-control" name="eZOption[{$attribute.id}][{$MultiOptionList.item.multioption_id}]" id="{$attribute.id}_{$group.group_id}_{$MultiOptionList.item.multioption_id}" onchange="ezmultioption_check_option( this, rules{$attribute.id}, {$attribute.id} );">
            {section var=Option loop=$MultiOptionList.item.optionlist}
